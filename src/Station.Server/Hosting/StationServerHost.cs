...
public static class StationServerHost
{
    ...
    public static async Task InitializeAsync(IServiceProvider services, CancellationToken token = default)
    {
        ...
        await scope.ServiceProvider.GetRequiredService<DatabaseUpgradeService>().UpgradeAsync(token);
    ...
    private static void AddServices(IServiceCollection services, StationOptions options, IPlayerAdapter? sharedPlayer)
    {
        ...
        services.AddScoped<IRoomLibraryRepository, EfRoomLibraryRepository>(); services.AddScoped<RoomLibraryService>(); services.AddScoped<IArtistBrowseService, EfArtistBrowseService>(); services.AddSingleton<RoomRealtimeJournal>(); services.AddSingleton<IRoomRealtimePublisher, SignalRRoomRealtimePublisher>(); services.AddSingleton<IQueueStatusNotifier, SignalRQueueStatusNotifier>();
    }
