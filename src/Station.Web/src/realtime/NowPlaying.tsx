...
export function NowPlaying() {
  ...
  const label = active?.title ?? (playbackState === 'Playing' || playbackState === 'Paused' ? '正在播放' : '等待播放')
  ...
  return <section className="playback-page" aria-label="正在播放">
    <header className="playback-hero"><span className="album"><Music2 aria-hidden="true" /></span><div><small>{playbackState === 'Paused' ? '已暂停' : active || playbackState === 'Playing' ? '正在播放' : '播放器待机'}</small><strong>{label}</strong></div></header>
