...
function readEnumString(data: unknown, name: string, values: string[]): string | undefined {
  const value = readValue(data, name)
  if (typeof value === 'string') return value
  return typeof value === 'number' && Number.isInteger(value) ? values[value] : undefined
}
...
function normalizePlayback(data: unknown): PlaybackProgress | undefined {
  if (!data || typeof data !== 'object') return undefined
  const state = readEnumString(data, 'state', ['Stopped', 'Idle', 'Preparing', 'Playing', 'Paused', 'Ended', 'Failed'])
  const position = readValue(data, 'position')
  if (!state || (typeof position !== 'string' && typeof position !== 'number')) return undefined
  const playbackId = readString(data, 'playbackId')
  const duration = readValue(data, 'duration')
  return { playbackId, state, position: String(position), duration: typeof duration === 'string' || typeof duration === 'number' ? String(duration) : undefined }
}

function normalizeSnapshot(data: unknown): RoomRealtimeSnapshot | undefined {
  if (!data || typeof data !== 'object') return undefined
  const version = readValue(data, 'version')
  const roomId = readString(data, 'roomId')
  const queueValue = readValue(data, 'queue')
  if (typeof version !== 'number' || !roomId || !Array.isArray(queueValue)) return undefined
  const queue = queueValue.map(normalizeQueueEntry).filter((item): item is QueueEntry => item !== undefined)
  const playbackValue = readValue(data, 'playback')
  return { version, roomId, queue, playback: playbackValue == null ? undefined : normalizePlayback(playbackValue) }
}
