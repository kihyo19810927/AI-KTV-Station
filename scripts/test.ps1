...
foreach ($project in $projects) {
    $arguments = @('test', $project.FullName, '--configuration', 'Release', '-p:UseSharedCompilation=false', '--settings', (Join-Path $root 'tests\coverage.runsettings'), '--collect', 'XPlat Code Coverage', '--results-directory', (Join-Path $root 'TestResults'), '--logger', 'console;verbosity=normal', '--filter', 'Category!=External&Category!=Performance')
