...
& $ffmpeg -hide_banner -loglevel error -y -t 10 -f lavfi -i color=c=blue:s=320x240:r=25:d=10 -f lavfi -i sine=frequency=440:duration=10 -f lavfi -i sine=frequency=880:duration=10 -i $srt -map 0:v -map 1:a -map 2:a -map 3:0 -map_metadata -1 -metadata:s:a:0 'title=伴奏' -metadata:s:a:1 'title=原唱' -metadata:s:s:0 'title=测试字幕' -c:v libx264 -pix_fmt yuv420p -c:a aac -c:s srt -t 10 -metadata:s:s:0 'language=zho' $Output
