...
`StationOptions` 是 `Station` 配置节的唯一强类型入口。默认监听 `0.0.0.0:5090`，数据目录使用相对路径 `data`，mpv、ffmpeg 和 ffprobe 由程序优先从发布包 `tools/` 自动定位。启动时验证端口、目录和播放器命令超时；无效配置阻止服务启动。
...
Windows 主控从当前用户的 `%LocalAppData%\AI-KTV Station\settings.json` 读取覆盖值。设置页可修改监听地址、端口、数据目录、mpv 可执行文件和 IPC 命令超时；保存前复用 `StationOptionsValidator`，监听地址必须是显式 IPv4/IPv6，变更在重启主控后生效。该文件属于主机私有状态，不应加入仓库。
